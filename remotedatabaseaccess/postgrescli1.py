#!/usr/bin/python
#------------------------------------------------
# Script name: postgrescli.py
#
# Description: 
# This script will run PostgreSQL database operations
#
# Pip packages needed:
# pip install --upgrade psycopg2 (Database driver)
#
# Parameters
# --host - PostgreSQL host (localhost, etc)
# --port - PostgreSQL port (5432, etc)
...
try: # Try to perform main logic
      ...
      ##  raise Exception(str(parmsexpected) + ' required parms - [PostgreSQL HOST] [PostgreSQL Port-Int] [PostgreSQL Database] [PostgreSQL User] [PostgreSQL Password]   [Action:QUERY/NONQUERY] [SQL Query] [OutputFile] [Replace-True/False] [Delimiter] [OutputType-CSV/JSON]. Process cancelled.')
      ...
      parser.add_argument('--database', required=True,help="PostgreSQL host")
      ...
      parmpass=parmpass.replace("\!","!") # Remove slach from \! if special char in password
      ...
      print(f"Connection info: Host-{parmhost} Port-{parmport} Database-{parmdatabase} User-{parmuser} Pass-{parmpass}")
      ...
      db=DbPostgres(parmhost,parmuser,parmpass,parmdatabase,parmport)
            ...
            if c1 != None:
              ...
              for row in rows:
                 ...
                 for col in row:
                     ...
                     rowdata = rowdata + str(row[curcol]).strip() + parmdelimiter
